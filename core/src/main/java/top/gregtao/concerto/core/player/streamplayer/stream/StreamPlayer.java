...
public class StreamPlayer implements StreamPlayerInterface, Callable<Void> {
	...
	/** Holds a list of Listeners to be notified about Stream PlayerEvents */
	 ...
	 * about the format of the audio data. This information include the sampling
	...
	private void createLine() throws LineUnavailableException, StreamPlayerException {
		...
		if (outlet.getSourceDataLine() != null)
			...
			final AudioFormat targetFormat = new AudioFormat(
					...
					nSampleSizeInBits / 8 * sourceFormat.getChannels(),
			...
			logger.info("Exited CREATE LINE()!:\n");
		}
	}
				...
				try {
					...
					if (status == Status.PLAYING) {
						...
						for (; toRead > 0 && (nBytesRead = audioInputStream.read(audioDataBuffer.array(), totalRead,
							toRead)) > 0; toRead -= nBytesRead, totalRead += nBytesRead)
	 ...
	 *
	 * @return The Position of the encoded stream in terms of bytes
	 */
	@Override
	 ...
	 *
	 * @param mute True to mute the audio
	 */
	@Override
